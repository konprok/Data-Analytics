...
parameters {
  ...
  real beta_insulin;  // współczynnik dla wieku
}
...
model {
  ...
  beta_insulin ~ normal(130, 60);  // Prior dla współczynnika wieku

  // Likelihood
  y ~ bernoulli(Phi(alpha + X[,1] * beta_glucose + X[,2] * beta_bmi + X[,3] * beta_insulin));
}